...
hyperparameters
'''
MODE = 'ali' # ali, alice, alice-z, alice-x, vegan
...
if MODE in ['vegan', 'vegan-wgan-gp']:
    ...
    def Discriminator(x, z):
        ...
        z_output = lib.ops.linear.Linear('Discriminator.2', 512, 512, z_output)
        z_output = LeakyReLU(z_output)
        z_output = tf.layers.dropout(z_output, rate=.2)
        ...
        output = lib.ops.linear.Linear('Discriminator.zx2', 512, 512, output)
        output = LeakyReLU(output)
        output = tf.layers.dropout(output, rate=.2)
...
def generate_image(frame, true_dist):
    ...
    lib.save_images.save_images(
        ...
        os.path.join(outf, '{}_mnist_samples_{}.png'.format(MODE, frame))
    )
...
def reconstruct_image(frame):
    ...
    lib.save_images.save_images(
        ...
        os.path.join(outf, '{}_mnist_reconstruction_{}.png'.format(MODE, frame))
    )
...
with tf.Session() as session:
    ...
    for iteration in xrange(ITERS):
        ...
        if iteration % 50000 == 49999:
            ...
            lib.visualization.scatter(data=z_dev_2D, label=np.hstack(y_dev), dir=outf, file_name='{}_mnist_manifold_{}.png'.format(MODE, iteration))

        # Save model
        if iteration == ITERS - 1:
            save_path = saver.save(session, os.path.join(outf, '{}_mnist_model_{}.ckpt'.format(MODE, iteration)))